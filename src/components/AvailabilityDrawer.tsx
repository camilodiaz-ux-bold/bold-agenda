...
export function AvailabilityDrawer({
  professionals, appointments, role, currentProfId, showProfSelector = true, onSave, onClose, onEditConflict,
}: Props) {
  const isAdmin = role === 'admin';
  const showProf = isAdmin && showProfSelector;
      ...
      <div className="flex-1 overflow-y-auto">
        <div className="px-5 pb-6 pt-2 flex flex-col gap-3">
          <div className="bg-[#FFFBEB] rounded-2xl px-4 py-3 flex gap-3">
            <AlertTriangle size={16} color="#B45309" strokeWidth={2} className="shrink-0 mt-0.5" />
            <div>
              <p className="text-sm font-bold text-[#B45309]">Conflicto con citas existentes</p>
              <p className="text-xs text-[#606060] mt-0.5 leading-relaxed">
                Este bloqueo coincide con {conflicts.length} cita{conflicts.length > 1 ? 's' : ''} ya agendadas. Puedes guardar el bloqueo y reprogramar después.
              </p>
            </div>
          </div>
          ...
          <div className="bg-white rounded-xl border border-gray-100 divide-y divide-gray-100">
            {conflicts.map(apt => {
              const svc = SERVICES.find(s => s.id === apt.serviceId);
              return (
                <div key={apt.id} className="px-4 py-2.5">
          ...
          <button
            ...
            className="w-full h-10 text-sm font-semibold text-[#121e6c] transition-opacity active:opacity-60"
          >
            Volver y ajustar
          </button>
        </div>
  ...
  return (
    <div className="flex flex-col flex-1 min-h-0">
      {/* Scrollable body */}
      <div className="flex-1 overflow-y-auto px-5 pt-1 pb-2 flex flex-col gap-3">

        {/* Professional (admin + showProfSelector only) */}
        {showProf && (
          <div>
